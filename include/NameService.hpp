...
struct NameService {
	...
	CommID destCommID = locateVertex(graph, destVertex);
	...
	CommID srcCommID = locateVertex(graph, srcVertex);
	Context context  = getGraphContext(graph);
	...
	CommID rootCommID = locateVertex(graph, rootVertex);
	CommID srcCommID  = locateVertex(graph, srcVertex);
	Context context   = getGraphContext(graph);
	std::vector<Vertex> vertices = getHostedVertices(graph, srcCommID); 
	...
	if(gathers[gatherID].send.size() == vertices.size()){
		...
		std::vector<Vertex> hostedVertices = getHostedVertices(graph, commID);
...
private:
    ...
    // Each graph is mapped to a context of the Communicator
