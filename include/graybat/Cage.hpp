...
template <typename T_CommunicationPolicy, typename T_GraphPolicy,
          typename T_SerializationPolicy =
              graybat::serializationPolicy::Forward>
struct Cage {
  ...
  using ContextID =
      graybat::communicationPolicy::ContextID<CommunicationPolicy>;
   ...
   *
   */
  auto announce(const std::vector<Vertex> &vertices, const bool global = true)
      -> void;
...
private:
   ...
   */
  template <class T>
  void reorder(const std::vector<T> &data,
    ...
    -> void {
        hostedVertices = distFunctor(communicator->getGlobalContext().getVAddr(),
    ...
    -> VAddr {
        ...
        if (it != vertexMap.end()) {
            ...
            errorMsg << "[" << communicator->getGlobalContext().getVAddr() << "] No host of vertex " << vertex.id << " known.";
    ...
    -> void {
        ...
        decltype(auto) serialized = SerializationPolicy::serialize(data);
        CPEvent e =
            communicator->asyncSend(destVAddr, edge.id, graphContext, serialized);
    ...
    auto Cage<T_CommunicationPolicy, T_GraphPolicy, SerializationPolicy>::recv(
      ...
      decltype(auto) skeleton = SerializationPolicy::prepare(data);
      CPEvent e =
          communicator->asyncRecv(srcVAddr, edge.id, graphContext, skeleton);

      events.push_back({e, [&data, skeleton]() {
                          SerializationPolicy::restore(data, skeleton);
                        }});
    }
