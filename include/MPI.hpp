...
namespace CommunicationPolicy {
	...
	void sendData(const T* const data, const size_t count, const CommUUID dest, const Context context, const MsgType msgType){
	    ...
	    MPI_Send(const_cast<T*>(data), count, MPIDatatypes<T>::type, dest, msgType, context.contextUUID);
	...
	Event asyncSendData(const T* const data, const size_t count, const CommUUID dest, const Context context, const MsgType msgType){
	    std::cout << "[" << uuidMap.at(context.contextUUID) << "] " << "MPI::ASend to " << dest << std::endl;
	    MPI_Request   request;
	    URI destURI = uriMap.at(context.contextUUID).at(dest);
	    MPI_Isend(const_cast<T*>(data), count, MPIDatatypes<T>::type, destURI, msgType, context.contextUUID, &request);
	...
	void recvData(T*  data, const size_t count, const CommUUID src, const Context context, const MsgType msgType){
	    ...
	    MPI_Recv(const_cast<T*>(data), count, MPIDatatypes<T>::type, srcURI, msgType, context.contextUUID, &status);
    
	}

	template <typename T>
	Event asyncRecvData(T*  data, const size_t count, const URI src, const Context context, const MsgType msgType){
	    std::cout << "[" << uuidMap.at(context.contextUUID) << "] " << "MPI::ARecv from " << src << std::endl;
	    MPI_Request   request;
	    URI srcURI = uriMap.at(context.contextUUID).at(src);
	    MPI_Irecv(const_cast<T*>(data), count, MPIDatatypes<T>::type, srcURI, msgType, context.contextUUID, &request);
	    return Event(request);
	...
	void reduce(const T* sendData, T* recvData, const size_t count, BinaryOperation op, const CommUUID root, const Context context){
	    ...
	    MPI_Reduce(const_cast<T*>(sendData), const_cast<T*>(recvData), count, MPIDatatypes<T>::type, op , rootURI, context.contextUUID);
	}

	template <typename T>
	void allReduce(const T* sendData, T* recvData, const size_t count, BinaryOperation op, const Context context){
	    MPI_Allreduce(const_cast<T*>(sendData), const_cast<T*>(recvData), count, MPIDatatypes<T>::type, op , context.contextUUID);
	...
	void broadcast(T* data, const size_t count, const CommUUID root, const Context context){
	    URI rootURI = uriMap.at(context.contextUUID).at(root);
	    MPI_Bcast(const_cast<T*>(data), count, MPIDatatypes<T>::type, rootURI, context.contextUUID);
	}
