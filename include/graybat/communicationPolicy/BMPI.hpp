...
#include <graybat/communicationPolicy/bmpi/Config.hpp>
#include <graybat/communicationPolicy/bmpi/Context.hpp>
#include <graybat/communicationPolicy/bmpi/Event.hpp>
#include <graybat/communicationPolicy/bmpi/Status.hpp>
...
template <> struct ContextType<BMPI> {
    using type = graybat::communicationPolicy::bmpi::Context<BMPI>;
};

template <> struct ContextIDType<BMPI> {
    using type = unsigned;
};

template <> struct EventType<BMPI> {
    using type = graybat::communicationPolicy::bmpi::Event;
};

template <> struct StatusType<BMPI> {
    using type = graybat::communicationPolicy::bmpi::Status;
};

template <> struct ConfigType<BMPI> {
...
struct BMPI : Base<BMPI> {
    ...
    template <typename T_Recv> Event recv(const Context context, T_Recv& recvData)
    {
        ...
        // auto status = context.comm.recv(mpi::any_source, mpi::any_tag, recvData.data(),
        // recvData.size());
...
*/
  ...
  private:
     ...
     *
     */
    template <typename T_Context> inline Uri getVAddrUri(const T_Context context, const VAddr vAddr)
    {
        Uri uri = 0;
        try {
            ...
            errorStream << "MPI::getVAddrUri::" << e.what() << " : Communicator with ID " << vAddr
