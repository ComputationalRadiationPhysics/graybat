...
std::vector<T_Data> randomVector(unsigned size){
...
int sendCAL(const unsigned N, const unsigned nSend, std::vector<double>& times) {
	...
	for(unsigned timestep = 0; timestep < N; ++timestep){
	    std::vector<T_Data> dataSend = randomVector<T_Data>(nSend);
	    switch(myVAddr) {
	    case 0:
...
int sendGVON(const unsigned N, const unsigned nSend, std::vector<double>& times) {
    ...
    std::vector<std::pair<Vertex, Edge>> outEdges = graph.getOutEdges(myGraphVertices[0]);
    std::vector<std::pair<Vertex, Edge>> inEdges = graph.getInEdges(myGraphVertices[0]);
